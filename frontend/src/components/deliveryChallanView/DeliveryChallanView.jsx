...
export function InvoiceView() {
  ...
  const { data, isLoading, isError, error, isFetching } = useQuery({
    ...
    queryFn: async () => {
      const deliveryChallanResponse = await axiosInstance.get(
        `/api/v1/deliveryChalan/view-delivery-chalan/${deliveryChallanId}`,
      );
    ...
    enabled: !!deliveryChallanId && isLoggedIn,
    staleTime: 5 * 60 * 1000,
  ...
  const handleDownload = () => {
    if (!deliveryChallan?.url) {
      setAlert("Invoice URL is not available for download.");

      return;
    }
    if (deliveryChallan?.url.includes("cloudinary")) {
      const url = deliveryChallan?.url?.replace(
        "/upload/",
        `/upload/fl_attachment:${userData?.businessName}-${deliveryChallan?.name}/`,
      );
      ...
      window.open(deliveryChallan?.url, "_blank");
    }
  };
        ...
        <div>
          ...
          <h4 className="mt-2 flex items-center gap-2 text-sm font-medium text-slate-500">
            ...
            <span className="rounded-md bg-slate-100 px-2 py-0.5 font-mono text-slate-700">
              {deliveryChallan?.name?.replace(".pdf", "")}
            </span>
          </h4>
        </div>
