...
export function NavBar() {
  ...
  return (
    <div>
      ...
      {/* FIXED: Moved overflow-y-auto here and used min-h-full inner flex to prevent top clipping */}
      {isInvoiceCredOpen && (
      ...
      {/* FIXED: Same fix as above. Ensures modal is scrollable on small screens */}
      {isItemAddOpen && (
        <div className="fixed inset-0 z-60 overflow-y-auto bg-slate-900/20 backdrop-blur-sm">
          <div className="flex min-h-full items-center justify-center p-4">
      ...
      <nav className="fixed left-1/2 top-6 z-40 flex w-[95%] max-w-5xl -translate-x-1/2 items-center justify-between rounded-full border border-slate-200/60 bg-white/80 px-2 py-2 shadow-2xl shadow-slate-200/20 backdrop-blur-xl transition-all">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          {isLoggedIn && (
            <Button
              ...
              // FIXED: Responsive padding (p-2 on mobile, px-3 py-2 on desktop)
            ...
            >
              {/* FIXED: Hidden text on mobile to save space */}
        ...
        <ul className="flex flex-1 items-center justify-center gap-1 overflow-x-auto px-2 no-scrollbar">
          {navItems.map(
            (item) =>
              item?.isVisible && (
                <li key={item?.name} className="shrink-0">
                  ...
                  >
                    {/* Optional: You could show Icons here for mobile if you have them, 
                      but text usually fits if buttons are small */}
                    {item?.name}
                  </NavLink>
                </li>
              )
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          {isLoggedIn && (
            <Button
              ...
              // FIXED: Icon only on mobile, Text on Desktop
